...
const filters: FilterCondition<User>[] = [
  FilterBuilder.field('roles', 'nin', 'user'),
]

const filterConfig: AdvancedFilter<User> = {
  where: FilterBuilder.and(...filters),
};

const userFilter = new Filter<User>();

const filteredUsers = userFilter.filter(users, filterConfig);
